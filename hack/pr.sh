...
if curl -XPOST -d "{\"title\":\"${BRANCH} automatic merge to master\", \"base\":\"master\", \"head\":\"${BRANCH}\", \"body\":\"${BRANCH} automatic merge to master\"}" \
  -s -o /dev/null -w "%{http_code}" https://api.github.com/repos/\
  "$(git remote -v | grep origin | head -n1 | awk '{print $2}' | cut -d':' -f2 | rev | cut -c 5- | rev)"/pulls \
