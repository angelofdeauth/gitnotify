...
func createLaunchdJobDarwin(u string) error {
	...
	template, err := box.TemplateBox.ReadEmbeddedTemplateToString("/service/darwin-launchd.plist.gotmpl")
	if err != nil {
	...
	if err := box.TemplateBox.WriteFileFromTemplate(template, path, mode, owners, interface{}); err != nil {
		if errr := box.EmbedDecoder.Reset(nil); errr != nil {
			return errr
		}
		return err
